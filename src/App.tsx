...
function App() {
  ...
  useEffect(() => {
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      if ((recognitionRef.current as any).processInput) {
        (recognitionRef.current as any).processInput.audioWorkletOptions = {
  ...
  const getRandomPositions = () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      do {
        ...
        attempts < 10 &&
        positions.some(pos => Math.abs(pos - left) < minDistance)
      );
  ...
  const checkAndPopBalloons = (spokenText: string) => {
